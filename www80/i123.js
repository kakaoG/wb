...
iweb.controller('i123', function ($scope,$filter) {

    $scope.backpack = {"pageSizeSelect":10,"pageNum":1,"pageSize":10,"searchType":'查询类型',"input":'',"day_num":-1,"start_end":[],"startTime":'',"endTime":'',"person_name":"", "person_id":""};
    $scope.data = {};
    ...
    $scope.selectTimes = function(time){
        ...
        $scope.backpack.start_end = [];
        $scope.backpack.pageNum = 1;
        $scope.iconState();
        $scope.flush();
    }
    ...
    $scope.selectQueryDate = function(){
        ...
        if(startTime!="" && endTime!=""){
            ...
            $scope.backpack.start_end=[dgt_startTime,dgt_endTime];
        }else{
    ...
    $scope.showList_child = function(num){
        $scope.backpack.pageSizeSelect = num;
        $scope.backpack.pageSize = $scope.backpack.pageSizeSelect;
        $scope.backpack.pageNum = 1;
        $scope.flush();
    }
    ...
    $scope.showList01 = function(){
        ...
        $('#type_list_ku').children().click(function(){
            $scope.backpack.searchType=$(this).html();
        });
    };
    ...
    //查询
    $scope.backpack_search_all = function(){
        $scope.backpack = {"pageSizeSelect":10,"pageNum":1,"pageSize":10,"searchType":'查询类型',"input":'',"day_num":-1,"start_end":[],"startTime":'',"endTime":'',"person_name":"", "person_id":""};
        $scope.flush();
    }

    $scope.backpack_search = function(){
        $scope.backpack.pageNum = 1;
        $scope.backpack.person_name ="";
        $scope.backpack.person_id ="";
        var a = {"玩家名字":"person_name","玩家ID":"person_id"};
        var key = a[$scope.backpack.searchType];
        $scope.backpack[key] = $scope.backpack.input;
        $scope.flush();
    }

    //上一页
    $scope.backpack_prev = function(){
        if($scope.backpack.pageNum>1){
            $scope.backpack.pageNum--;
            $scope.flush();
        }
    }
    //下一页
    $scope.backpack_next = function(){
        if($scope.backpack.pageNum<$scope.data.total_page){
            $scope.backpack.pageNum++;
            $scope.flush();
        }
    }
    ...
    $scope.export_xlsx = function(){
        ...
        var records = $scope.backpack.pageSize*$scope.data.total_page;
        apiconn.send_obj({
            "obj": "poolRecord",
            "act": "detail",
            ...
            "event":"pack",
            "condition":{
                "day_num":$scope.backpack.day_num,//要查看的最近的天数(0:今天|1:昨天|7:近7日|30:近30日|-1:全部),字段没有则表示没有该检索条件
                "start_end":$scope.backpack.start_end, //为[]或者字段没有则表示没有该检索条件，否则以start_end为准忽略day_num条件
                "person_name":$scope.backpack.person_name,
                "person_id":$scope.backpack.person_id
            },
            "page_num":0,
            "page_size":records
        });
    ...
    $scope.flush = function(){
        ...
        apiconn.send_obj({
            ...
            "page_num":$scope.backpack.pageNum-1,
            "page_size":$scope.backpack.pageSize
        });
    }
