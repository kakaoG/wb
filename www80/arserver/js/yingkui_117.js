function yingkui_117_(propdata){
    $("#game").style["min-width"]="900px";
    ...
    function buildDataList(){
        ...
        var titleArr=["日期","操作用户","金额变化"];
        var lineW=[200,300,200],ws=0;
...
text+='<div class="lists" style="width:'+lineW[2]+'px;">'+data.val+'</div>';
                
            }else{
                text+='<div class="lists" style="width:'+lineW[0]+'px;">'+forMatime(data.ut*1000)+'</div>';
text+='<div class="lists" style="width:'+lineW[1]+'px;">'+data.person_name+'</div>';
text+='<div class="lists" style="width:'+lineW[2]+'px;">'+data.money_change+'</div>';
...
text+='<div class="lists" style="width:'+lineW[2]+'px;">'+data.money_change+'</div>';
    ...
    function start(){
        ...
        exchange_div4_3.addEventListener("click",function(e){
            doLastSend(function(data){
                ...
                downloadFile(propdata.title+".csv",text);
           });
 ...
 server.merchant_add_update(currOpenEdit,merchant_name,merchant_addr,phone,merchant_id,function(data){
    ...
    ajax("html/yingkui_117.html",function(text){
        $("#gamebottomdiv").innerHTML=text;
        $("#gamebottomdiv .layer_title span").innerHTML=propdata.title;
        start();
    });
