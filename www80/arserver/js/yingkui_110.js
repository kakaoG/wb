...
text+='<div class="lists" style="width:'+lineW[4]+'px;">'+data.current_money+'</div>';
        ...
        for(var i=0;i<listedit_2.length;i++){
            listedit_2[i].addEventListener("click",function(){
                ...
                if(eventType[key]){
                    ...
                    //自定义类型
                    window.runListMove("yingkui_117",{
                        "key":key,
                        "title":title
                    });
                }
    ...
    function start(){
        ...
        exchange_div2_6.addEventListener("click",function(e){
            server.pool_gain_events("pool_gain",function(data){
                event_list={};
                if(data.event_list){
                ...
                $("#rolenum_pop_3").style.display="block";
               $("#rolenum_pop_4").style.display="block";
                $("#rolenum_pop_1").innerHTML="事件信息编辑";
                 $("#rolenum_pop_2 span").innerHTML="事件描述";
                 $("#rolenum_pop_2 input").placeholder="事件描述";
                $("#rolenum_pop_2 input").value="";
                $("#exchangebus_pop1").style.height="320px";
                $("#rolenum_pop_4 input").value=0;
                openType=1;
            });
        ...
        //注入行为池
        var exchange_div2_7=$("#exchange_div2_7");
        exchange_div2_7.addEventListener("click",function(e){
           $("#rolenum_pop").style.display="block";
           $("#bottom_left .share_pop").style.display="block";
            $("#rolenum_pop_3").style.display="none";
            $("#rolenum_pop_4").style.display="none";
            $("#rolenum_pop_1").innerHTML="注入行为池";
            $("#rolenum_pop_2 span").innerHTML="注入金额";
            $("#rolenum_pop_2 input").placeholder="注入金额";
            $("#rolenum_pop_2 input").value="";
            $("#exchangebus_pop1").style.height="200px";
            openType=2;
        });
        
        //注入基金池
        var exchange_div2_6_1=$("#exchange_div2_6_1");
        exchange_div2_6_1.addEventListener("click",function(e){
           $("#rolenum_pop").style.display="block";
           $("#bottom_left .share_pop").style.display="block";
           $("#rolenum_pop_3").style.display="none";
           $("#rolenum_pop_4").style.display="none";
            $("#rolenum_pop_1").innerHTML="注入基金池";
             $("#rolenum_pop_2 span").innerHTML="注入金额";
            $("#rolenum_pop_2 input").placeholder="注入金额";
            $("#rolenum_pop_2 input").value="";
            $("#exchangebus_pop1").style.height="200px";
        ...
        //输入内容
        var eventclewinput=$("#eventclewinput");
        eventclewinput.addEventListener("blur",function(e){
           $("#eventclewdiv").style.display="none";
        });
        ...
        function buildclew(){
            ...
            var val=eventclewinput.value;
            var arr=[],li="";
            //获取内容生产列表
            for(var i in event_list){
                if(i.match(val)){
                    arr.push(i);
                    li+='<li data-type="'+i+'">'+i+'</li>';
                    if(arr.length>=6){
                        break;
                    }
                }
            ...
            if(arr.length==0){
                ...
                $("#eventclewdiv").style.display="block";
                $("#eventclewdiv").innerHTML=li;
                var li=$("#eventclewdiv li",true);
                for(var i=0;i<li.length;i++){
                    li[i].addEventListener(mousedown,function(e){
                        e.stopPropagation();
                        e.preventDefault();
                        eventclewinput.value=this.getAttribute("data-type");
                        $("#eventclewdiv").style.display="none";
                    });
                }
            }
        }
        eventclewinput.addEventListener("focus",function(e){
           buildclew();
        });
        eventclewinput.addEventListener("keyup",function(e){
            buildclew();
        });
        ...
        btn_save.addEventListener("click",function(e){
            if(openType==1){
                var isadd=true;
                if($("#yinkui_remove_radio").checked){
                    isadd=false;
                }
                //事件信息编辑
                var vals=$("#rolenum_pop_4 input").value;
                vals=parseInt(vals);
                if(vals>0){
                ...
                $("#rolenum_pop_4 input").value=vals;
                if(!isadd){
                    vals=-vals;
                }
                var event=eventclewinput.value;
                if(event==""){
                    alert("输入事件描述");
                    return;
                }
                //isadd
                server.pool_gain_injact(event,vals,function(d){
                    if(d.status=="success"){
                        $("#rolenum_pop").style.display="none";
                        $("#bottom_left .share_pop").style.display="none";
                        //查询
                ...
                eventclewinput.value=vals;
                //注入行为池 
                server.pool_gain_injact("注入行为池",vals,function(d){
                    if(d.status=="success"){
                        $("#rolenum_pop").style.display="none";
                        $("#bottom_left .share_pop").style.display="none";
                        //查询
                ...
                eventclewinput.value=vals;
                //注入行为池 
                server.pool_gain_injact("注入基金池",vals,function(d){
                    if(d.status=="success"){
                        $("#rolenum_pop").style.display="none";
                        $("#bottom_left .share_pop").style.display="none";
                        //查询
    ...
    ajax("html/yingkui_110.html",function(text){
